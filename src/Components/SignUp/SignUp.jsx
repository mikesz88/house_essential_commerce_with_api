...
class SignUp extends React.Component {
    ...
    passwordValidationCheck = (value) => {
        ...
        return !error
            ? "This does not fit the requirement. Try again!"
            : undefined;
    };
    ...
    emailAlreadyTaken = (value) => {
        ...
        const alreadyTaken = users.some(
            (user) => this.props.users[user]["email"] === value
        );
        return alreadyTaken
    ...
    emailCheck = (value) => {
        ...
        return !error
            ? "This is not a proper email. Try again!"
    ...
    handleValidations = (target, value) => {
        ...
        switch (target) {
            case "email":
                ...
                this.setState((prevState) => ({
                    error: { ...prevState.error, email: errorText },
                }));
                break;
            case "password":
                errorText = this.passwordValidationCheck(value);
                this.setState((prevState) => ({
                    error: { ...prevState.error, password: errorText },
                }));
                break;
            case "confirmPassword":
                errorText = this.passwordMatchCheck(value);
                this.setState((prevState) => ({
                    error: { ...prevState.error, confirmPassword: errorText },
                }));
                break;
            case "firstName":
            ...
            <form onSubmit={this.handleSubmit} className={style.flexContainer}>
                <div className={style.loginHeader}>
                    ...
                    {generalError ? (
                        <div
                            id="generalError"
                            className={`${style.generalError}`}
                        >
                            We're sorry, but one or more fields are incomplete
                            or incorrect. <u>Find error(s)</u>.
                        </div>
                    ) : null}
                </div>
                ...
                <div className={`${style.inputContainer}`}>
                    ...
                    <p className={`${style.finePrint}`}>
                        ...
                        one capital letter, at least one small letter, one
                ...
                <button className={`btn round-pill ${style.submitBtn}`}>
                    Submit
                </button>
            </form>
        );
